...
struct SoundPrintView: View {
        ...
        NavigationStack {
            ZStack {
                ...
                if !authService.isAuthorized() {
                    // Show connect button when Spotify is not authenticated
                    connectSpotifyView
            ...
            .task {
                // Only load if Spotify is authenticated
                if authService.isAuthorized() {
                    await loadData()
                    await loadInsights()
                } else {
            ...
            .onChange(of: authService.isAuthorized()) { oldValue, newValue in
                if newValue {
                    Task {
                        await loadData()
                        await loadInsights()
    ...
    private var connectSpotifyView: some View {
        VStack(spacing: 32) {
            ...
            VStack(spacing: 16) {
                ...
                HStack(spacing: 4) {
                    ForEach([0.8, 0.6, 0.4, 0.3, 0.5, 0.7, 0.9, 0.7, 0.5, 0.3, 0.4, 0.6, 0.8], id: \.self) { height in
                        RoundedRectangle(cornerRadius: 2)
                            .fill(Color.white)
                            .frame(width: 4, height: CGFloat(height * 40))
                    }
                }
                .padding(.top, 8)
            }
            ...
            VStack(spacing: 16) {
                ...
                Text("Connect your Spotify account to discover your music identity, see your top artists, and explore your SoundPrint.")
                    .font(.subheadline)
    ...
    func loadData() async {
        ...
        guard authService.isAuthorized() else {
            await MainActor.run {
                isLoading = false
                errorMessage = "Please connect your Spotify account to view your SoundPrint"
            }
            return
        }
